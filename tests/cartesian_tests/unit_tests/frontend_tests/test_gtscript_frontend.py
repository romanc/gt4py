...
class TestInlinedExternals:
    ...
    def test_literal_precision(self):
        def stencil(input_field: gtscript.Field[float], output_field: gtscript.Field[float]):
            from __externals__ import my_int, my_float, my_int32, my_int64, my_float32, my_float64

            with computation(PARALLEL), interval(...):
                output_field = input_field + my_int
                output_field = output_field + my_float
                output_field = output_field + my_int32
