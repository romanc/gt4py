...
class LocNode(eve.Node):
    loc: eve.SourceLocation | None = None
...
class Literal(eve.Node):
    ...
    # maybe it should be float | int | str etc?
...
class FieldAccess(eve.GenericNode, Generic[ExprT, VariableKOffsetT]):
    ...
    @classmethod
    def centered(cls, *, name: str, loc: eve.SourceLocation | None = None) -> FieldAccess:
...
class BinaryOp(eve.GenericNode, Generic[ExprT]):
    ...
    op: ArithmeticOperator | ComparisonOperator | LogicalOperator
    left: ExprT
    right: ExprT
...
class _LvalueDimsValidator(eve.VisitorWithSymbolTableTrait):
    ...
    def visit_Node(
        self, node: eve.Node, *, loop_order: LoopOrder | None = None, **kwargs: Any
    ) -> None:
...
class HorizontalInterval(eve.Node):
    ...
    @classmethod
    def at_endpt(
        cls, level: LevelMarker, start_offset: int, end_offset: int | None = None
    ) -> HorizontalInterval:
