...
from gt4py.cartesian.gtc.dace import (
    daceir as dcir,
    oir_to_tasklet,
    treeir as tir,
    treeir_context as tctx,
)
...
class OIRToTreeIR(eve.NodeVisitor):
    def visit_CodeBlock(self, node: oir.CodeBlock, ctx: tctx.TreeIRContext) -> None:
    ...
    def _insert_evaluation_tasklet(
        self, node: oir.MaskStmt | oir.While, ctx: tctx.TreeIRContext
    ...
    def visit_HorizontalExecution(
        self, node: oir.HorizontalExecution, ctx: tctx.TreeIRContext
    ) -> None:
        # TODO
    ...
    def visit_MaskStmt(self, node: oir.MaskStmt, ctx: tctx.TreeIRContext) -> None:
        ...
        with if_else.open(ctx):
            groups = self._group_statements(node)
            self.visit(groups, ctx=ctx)

    def visit_HorizontalRestriction(
        self, node: oir.HorizontalRestriction, ctx: tctx.TreeIRContext
    ) -> None:
    ...
    def visit_HorizontalMask(self, node: common.HorizontalMask, ctx: tctx.TreeIRContext) -> str:
    ...
    def visit_While(self, node: oir.While, ctx: tctx.TreeIRContext) -> None:
    ...
    def visit_VerticalLoopSection(
        self, node: oir.VerticalLoopSection, ctx: tctx.TreeIRContext, loop_order: common.LoopOrder
    ) -> None:
        bounds = self.visit(
            node.interval,
    ...
    def visit_VerticalLoop(self, node: oir.VerticalLoop, ctx: tctx.TreeIRContext) -> None:
        if node.caches:
