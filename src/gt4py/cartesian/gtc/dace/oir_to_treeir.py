...
class OIRToTreeIR(eve.NodeVisitor):
    ...
    def visit_MaskStmt(self, node: oir.MaskStmt, ctx: tir.Context) -> None:
        # Use a Tasklet to render `node.mask` into a string of python code
        tmp_ctx = oir_to_tasklet.Context(
            code=[], targets=set(), inputs={}, outputs={}, tree=ctx.root, scope=ctx.current_scope
        )
        code = oir_to_tasklet.OIRToTasklet(in_prefix="", out_prefix="").visit(
            node.mask, ctx=tmp_ctx, is_target=False
        )

        if_else = tir.IfElse(if_condition_code=code, children=[], parent=ctx.current_scope)
